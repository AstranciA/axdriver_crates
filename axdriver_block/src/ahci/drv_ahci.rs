...
fn ahci_print_info(ahci_dev: &ahci_device) {
    ...
    let speed_s = if speed == 1 {
        ...
        debug!("AHCI vers {}.{}.{}.{}, {} slots, {} ports, {} Gbps, 0x{} impl, {} mode",
            vers >> 24 & 0xff,
            vers >> 16 & 0xff,
            vers >> 8 & 0xff,
            vers & 0xff,
...
fn ahci_sata_print_info(pdev: &ahci_blk_dev) {
    let serial = unsafe { CStr::from_bytes_with_nul_unchecked(&pdev.serial) };
    let product = unsafe { CStr::from_bytes_with_nul_unchecked(&pdev.product) };
    let revision = unsafe { CStr::from_bytes_with_nul_unchecked(&pdev.revision) };
    debug!(
...
fn ahci_host_init(ahci_dev: &mut ahci_device) -> i32 {
    ...
    for i in 0..ahci_dev.n_ports {
        ...
        if timeout <= 0 {
            debug!("port {} sata link timeout", i);
        } else {
            debug!("port {} sata link up", i);
        }
...
fn ahci_exec_ata_cmd(
    ...
    if cmd_slot == 32 {
        debug!("cannot find empty command slot");
        return 0;
    }

    if buf_len > AHCI_MAX_BYTES_PER_TRANS {
        debug!("max transfer length is {} bytes", AHCI_MAX_BYTES_PER_TRANS);
        return 0;
    }
...
fn ahci_port_start(ahci_dev: &mut ahci_device, port: u8) -> i32 {
    ...
    if timeout <= 0 {
        debug!("ahci port {} failed to start", port);
        return -1;
    }
    return 0;
}
...
pub fn ahci_sata_flush_cache_ext(ahci_dev: &ahci_device) {
    let port: u8 = ahci_dev.port_idx;
    let cfis: sata_fis_h2d = sata_fis_h2d {
        fis_type: SATA_FIS_TYPE_REGISTER_H2D,
        pm_port_c: 0x80,
