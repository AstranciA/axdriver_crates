...
macro_rules! spin_wait {
    ($e:expr) => {
        // 使用一个循环，直到表达式 $e 求值为 true
        while !$e {
            //axtask::sleep(Duration::new(0, 1000000));
            // 插入编译器提示，表明这是一个自旋循环。
            // 在支持的架构上，这可能被翻译成 CPU 友好的指令（如 x86 的 PAUSE）。
            // 它可以减少 CPU 功耗和提高超线程环境下的性能。
            core::hint::spin_loop();

            // 插入一个编译器内存屏障。
            // Ordering::Acquire 确保在屏障之后的内存读取不会被重排到屏障之前。
            // 这对于等待共享变量状态改变的自旋循环至关重要，
            // 确保我们每次检查 $e 时都能看到最新的值。
...
impl MMC {
    ...
    pub fn card_init(&self) {
        ...
        loop {
            ...
            axtask::sleep(Duration::new(0, 100000)); // Wait for card to power up
        }
        ...
        /*
         *let descriptor_frame = alloc_frame(false).expect("Error allocating descriptor page");
         *let descriptor_page_paddr: usize = descriptor_frame.pa.as_usize();
         ...
         *    buffer_page_paddr[i] = alloc_frame(false)
         *        .expect("Error allocating buffer page")
         *        .pa
         *        .as_usize();
         *}
    ...
    fn send_cmd(
        ...
        if cmd.data_expected() {
            ...
            if is_read {
                spin_wait!({
                    ...
                    if rinsts.receive_data_request() && !self.dma_enabled() {
                        while buffer_offset * 8 < 512 && self.fifo_filled_cnt() >= 2 {
                ...
                spin_wait!({
                    ...
                    if rinsts.transmit_data_request() && !self.dma_enabled() {
                        ...
                        while buffer_offset * 8 < 512 && self.fifo_filled_cnt() < 120 {
            ...
            trace!("transmit {:?} bytes", (buffer_offset) * 8);
            trace!("Current oFIFO count: {}", self.fifo_filled_cnt());
    ...
    fn virt_base_address(&self) -> usize {
        phys_to_virt(PhysAddr::from_usize(self.base_address)).as_usize()
    }
}

impl BaseDriverOps for MMC {
...
impl BlockDriverOps for MMC {
    fn num_blocks(&self) -> u64 {
        // Hardcoded default for testing
        // TODO: Implement proper detection from CSD register
        64 * 1024 * 1024 * 1024 / self.block_size() as u64 // 32GB card
    }

    fn block_size(&self) -> usize {
        let base = self.virt_base_address() as *mut BLKSIZ;
        unsafe { base.byte_add(BLKSIZ::offset()).read_volatile() }.block_size()
    }

    #[allow(mutable_transmutes)]
    fn read_block(&mut self, block_id: u64, buf: &mut [u8]) -> DevResult {
        let buf = unsafe { core::mem::transmute(buf) };
        trace!("reading block {}", block_id);
        // Read one block
        self.set_size(512, 512);
